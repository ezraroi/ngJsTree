...
angular.module('ngJsTree').directive('jsTree', function($log) {
    return {
        restrict: 'A',
        scope: {
            treeData:'=ngModel'
        ...
        link: function(scope, elm, attrs,controller) {
            ...
            nodesWatcher.onChanged = function(node) {
                $log.debug('jsTree: onChanged called');
            ...
            nodesWatcher.onAdded = function(node) {
                //$log.debug('jsTree: onAdded called');
                var parent = scope.tree.jstree(true).get_node(node.parent);
                scope.tree.jstree(true).create_node(parent,node, 'inside', angular.noop, false );
            };

            nodesWatcher.onRemoved = function(node) {
                scope.tree.jstree(true).delete_node(node.id);
                //$log.debug('jsTree: onRemoved called');
            };
